...
import { PiUsersThreeLight, PiSuitcaseSimple, PiBellRinging } from "react-icons/pi"; // People, Bag, Bell
import { LuMessageCircleMore, LuSearch } from "react-icons/lu"; // Message, Search
    ...
    return (
        <nav className='h-16 py-2 w-full px-2 md:px-8 lg:px-16 bg-white flex gap-4 justify-between items-center fixed top-0 left-0 right-0 z-50'>
            ...
            <div className='relative w-full lg:w-fit'>
                <input
                    type="text"
                    className='h-10 w-full lg:w-64 pl-3 md:pr-10 text-sm bg-[#F6F6F6] outline-none rounded-lg lg:rounded-2xl placeholder:text-sm'
                    placeholder='Search for anything (Jobs)'
                />
                <button
                    name='search button'
                    className='h-10 w-10 pr-1 rounded-r-lg lg:rounded-r-2xl text-white bg-blue-400 absolute right-0 top-0 bottom-0 flex justify-center items-center'
                >
                    <LuSearch />
                </button>
            </div>
